...
// --- 硬件与系统参数配置 ---
#define AD9834_VIN_VPP      0.100f      // 差分放大器后的实际正弦波输入幅度: 100mV
#define VOUT_MIN            0.02f       // 最小输出电压极限: 20mV (0.02V)
#define VOUT_MAX            7.0f        // 最大输出电压极限: 7.0V
...
void AmplitudeControl_SetVoltage(float targetVpp) {
    ...
    // 如果设置目标为 2.0V 时，之前 64.539 截距输出了 2.35V。
    // 改为 63.125 后输出反而变成了 2.7V，说明该模块的控制极性或运算符号与预期相反。
    // 既然减小截距导致输出上升，那么我们需要增大截距来压低输出。
    // 之前: 64.539 -> 2.35V
    // 刚才: 63.125 -> 2.70V (截距降低1.4，输出上升了约0.35V)
    // 我们现在将截距增大，往相反方向校准。
    float vda_mv = (65.95f - gain_db) / 0.0402f;
